// Lightning RAG avec Google Gemini 2.5 - API Key fonctionnelle!
...
class Gemini25LightningRAG {
    ...
    // Embeddings Gemini 2.5 avec cache
    async getGeminiEmbedding(text) {
        ...
        try {
            ...
            console.log('Gemini 2.5 embedding généré:', embedding.length, 'dimensions');
            
            return embedding;
        } catch (error) {
            console.warn('Gemini 2.5 embedding failed, using fallback');
            // Fallback simple pour développement
            return Array(768).fill(0).map(() => Math.random() - 0.5);
        }
    }

    // Recherche locale optimisée (pas Pinecone pour le moment)
...
// API endpoint Gemini 2.5 Lightning
